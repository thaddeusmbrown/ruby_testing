...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      it 'returns 7' do
        game = described_class.new(5, 9, number_guessing)
        solution = game.make_guess
    ...
    context 'when min is 8 and max is 9' do
      it 'returns 8' do
        game = described_class.new(8, 9, number_guessing)
        solution = game.make_guess
        expect(solution).to eq(8)
      end
    end
    ...
    context 'when min is 0 and max is 3' do
      it 'returns 1' do
        game = described_class.new(0, 3, number_guessing)
        solution = game.make_guess
    ...
    context 'when min and max both equal 3' do
      it 'returns 3' do
        game = described_class.new(3, 3, number_guessing)
        solution = game.make_guess
        expect(solution).to eq(3)
      end
    end
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:number_guessing) { double('number_guessing', value: 8) }
      subject(:game_over) { described_class.new(0, 9, number_guessing, 8) }
    ...
    context 'when guess and random_number are not equal' do
      let(:number_guessing) { double('number_guessing', value: 8) }
      subject(:game_not_over) { described_class.new(0, 9, number_guessing, 7) }
      it 'is not game over' do
        solution = game_not_over.game_over?
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'updates min to 5' do
        ...
        solution = low_guess_game.min
        expect(solution).to eq(5)
      end

      it 'does not update max' do
        low_guess_game.update_range
        solution = low_guess_game.max
    ...
    context 'when the guess is more than the answer' do
      ...
      it 'does not update min' do
        ...
        solution = high_guess_game.min
        expect(solution).to eq(0)
      end

      it 'updates max to 8' do
        high_guess_game.update_range
        solution = high_guess_game.max
        expect(solution).to eq(8)
      end
    end
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      subject(:close_guess_game) { described_class.new(5, 8, number_guessing, 7) }
      it 'updates min to the same value as max' do
        close_guess_game.update_range
        min_new = close_guess_game.min
        max_new = close_guess_game.max
        expect(min_new).to eq(max_new)
      end

      it 'does not update max' do
        close_guess_game.update_range
        solution = close_guess_game.max
        expect(solution).to eq(8)
      end
    end
  end
